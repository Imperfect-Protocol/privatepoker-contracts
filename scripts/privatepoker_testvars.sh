...
deposit_test_usdc() {
    load_privatepoker_env 1 || return 1
    load_privatepoker_test_usdc_env || return 1
    load_privatepoker_core_env || return 1

    if [ "$#" -ne 1 ]; then
        cat >&2 <<EOF
Usage: ./scripts/deposit-test-usdc.sh AMOUNT_CENTS

Approves TEST_USDC to the configured Private Poker Cashier and deposits it
to mint equivalent Chips to the wallet configured by PP_PRIVATE_KEY.

AMOUNT_CENTS is a dollar-cent amount. TEST_USDC has 6 decimals, so:
  ./scripts/deposit-test-usdc.sh 100000
deposits 1000.00 TEST_USDC and mints equivalent Chips to that wallet.
EOF
        return 1
    fi
    ...
    require_env PP_PRIVATE_KEY || return 1
    require_env PP_TEST_USDC || return 1
    require_env PP_CASHIER || return 1

    raw_amount=$((amount_cents * 10000))
    depositor=$(cast wallet address "$PP_PRIVATE_KEY") || return 1

    log_step "Approving Cashier ${PP_CASHIER} to spend ${amount_cents} cents (${raw_amount} raw TEST_USDC units) from ${depositor}"
    contract_send "$PP_TEST_USDC" 'approve(address,uint256)' "$PP_CASHIER" "$raw_amount" || return 1

    allowance=$(contract_call "$PP_TEST_USDC" 'allowance(address,address)(uint256)' "$depositor" "$PP_CASHIER" | awk '{ print $1 }') || return 1
    if [ "$allowance" -lt "$raw_amount" ]; then
        die "TEST_USDC allowance for Cashier is still too low: allowance=$allowance required=$raw_amount"
        return 1
    fi

    log_step "Depositing TEST_USDC from ${depositor} into Cashier and minting Chips to msg.sender"
    contract_send "$PP_CASHIER" 'deposit(uint256)' "$raw_amount" || return 1
}
